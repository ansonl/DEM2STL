...
$env:PYTHONPATH = "$env:TOPDIR/templates/pr-sqrt/"
...
gdalbuildvrt -resolution highest -overwrite globalScaleLandA.vrt $env:SOURCES/pr_srtm_32620_1000m_avg_250m_cubicspline.tif
...
gdalbuildvrt -resolution highest -overwrite raiseLandAIfNotInHydroMaskBAndScale.vrt globalScaleLandA.tif $env:HYDROGRAPHICMASKS/pr_coastline_32620_250m_int16.tif
...
gdalbuildvrt -resolution highest -overwrite keepLandAIfNotInHydroMaskB.vrt globalScaleLandA.tif $env:HYDROGRAPHICMASKS/pr_coastline_32620_250m_int16.tif
...
#gdal
python ./generate-batch.py .\templates\pr-sqrt\pr-sqrt.json -gc

#tt
python ./generate-batch.py .\templates\pr-sqrt\pr-sqrt.json -gt