...
with open('./'+batchFilename, 'w+') as cmdfp:
    ...
    for entry in os.scandir("./cb_2019_us_states_individual"):
    	if entry.name.endswith('.gpkg') and entry.is_file():
    		#print(entry.path)
            
            stateName = entry.name.replace(".gpkg","")
    
            clipElevationCmd = f'gdalwarp -overwrite -t_srs ESRI:102004 -of GTiff -tr 100.0 100.0 -tap -cutline {entry.path} -crop_to_cutline ./dems/7-5-arc-second-merged.tif ./dems/7-5-arc-second-clipped-100m/{stateName}.tif -multi'

            clipMaskCmd = f'gdalwarp -overwrite -t_srs ESRI:102004 -of GTiff -tr 100.0 100.0 -tap -cutline {entry.path} -crop_to_cutline ./usa_hydro1k_hydrolakes_merged/usa_hydro1k_hydrolakes_merged_100m.tif C:/Users/ansonl/development/dem-to-stl-workflow/dems/stream-lake-mask-clipped-100m/{stateName}.tif -multi'
